Ext.define('CBH.view.sales.FileQuoteConfirmation', {
    ...
    initComponent: function() {
        ...
        var storeVendorCarrier = null,
            storeCustContact = null;
    ...
    onRenderForm: function() {
        ...
        formsvinfo.down('field[name=FVPaymentTerms]').bindStore(new CBH.store.common.PaymentTerms().load({
            params: {
                page: 0,
                start: 0,
                limit: 0
    ...
    onAfterLoadRecord: function(record) {
        ...
        me.up('app_viewport').getEl().mask('Please wait...');
        ...
        var storeCurrentVendor = new CBH.store.vendors.Vendors().load({
            ...
            callback: function() {
                ...
                me.down('field[name=FileCurrentVendor]').setValue(curRec.FileCurrentVendor);
                me.storesLoaded.push(this);
                me.checkLoaded();
            }
        });
            ...
            callback: function(records, success, eOpts) {
                if(records[0]) {
                    ...
                    this.each(function(record) {
                        /*var fieldText = me.down('field[name=txt' + record.get('QCDGLAccount') + "]");
                        if(record.data.Estimated) {
                            fieldText.setValue("~ " + Ext.util.Format.usMoney(record.Cost) + " ~");
                        } else {
                            fieldText.setValue(Ext.util.Format.usMoney(record.Cost) + '"');
                        }*/
                    ...
                    me.down("field[name=txtOtherCost]").setValue(Ext.util.Format.usMoney(curOtherCostSum));
                    me.down("field[name=txtOtherPrice]").setValue(Ext.util.Format.usMoney(curOtherPriceSum));
                }
                me.storesLoaded.push(this);
                me.checkLoaded();
            }
    ...
    afterCleanupFileQuoteVendorInfo: function() {
        ...
        if (String.isNullOrEmpty(record.data.QHdrInspectionNum)) {
            ...
            if (record.data.QHdrInspectorKey === null) {
                Ext.Msg.show({
                    title:'Conversion Aborted!',
                    msg: 'You must select an inspection company if there is an inspection number',
                    buttons: Ext.Msg.OK,
                    fn: function() {
            ...
            success: function(response) {
                ...
                if(data.success) {
                    Ext.Msg.show({
                        ...
                        msg: 'The FOB Value for this quote requires an inspection for the country the product is being shipped to.  (Or no country has been specified)  Are you sure you want to continue with the conversion?',
                        buttons: Ext.Msg.YESNO,
                        icon: Ext.Msg.QUESTION,
                        fn: function(btn) {
                            if (btn === "yes") {
    ...
    sendMail: function(msg) {
        var me = this,
            ...
            cc = msg.RecipientsCC.length >  0 ? msg.RecipientsCC.join().replace('"','') : "",
            subject = msg.Subject;
