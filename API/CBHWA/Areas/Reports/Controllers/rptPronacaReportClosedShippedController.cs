namespace CBHWA.Areas.Reports.Controllers
{
    ...
    public class rptPronacaReportClosedShippedController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            string strWhere = nvc["strWhere"] ?? "rptDate > CAST('2012-12-31' AS DATE) AND rptDate < CAST('2013-04-09' AS DATE) AND JobIsClosed = 1";
        ...
        {
            var nvc = Request.QueryString;
            strWhere = strWhere ?? "rptDate > CAST('2012-12-31' AS DATE) AND rptDate < CAST('2013-04-09' AS DATE) AND JobIsClosed = 1";
            ...
            lr.ReportPath = "Areas/Reports/ReportDesign/rptPronacaReportClosedShipped.rdlc";
            ...
            return File(bytes, "application/pdf", "rptPronacaReportClosed");
        }
            ...
            using (SqlConnection oConn = ConnManager.OpenConn())
            {
                string sql = @"WITH qData
                                ...
                                (
                                    ...
                                    ,ISNULL((CASE WHEN a.JobPriceUSD = 0 Then 0 Else (a.JobProfit/a.JobPriceUSD) End),0) as ProfitPercent
                                   FROM qrptPronacaReport a
                                   WHERE {0} 
                                )
                                SELECT a.*
                                    ,ROW_NUMBER() OVER (ORDER BY a.CustKey, a.rptDate, a.FileNum) as Row
                                FROM qData a
                                ORDER BY Row

                                DELETE FROM tblReportCriteria WHERE CriteriaRptName = 'rptPronacaReportClosedShipped' and CriteriaEmployeeKey = {1};";
