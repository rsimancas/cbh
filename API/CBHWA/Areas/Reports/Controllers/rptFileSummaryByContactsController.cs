namespace CBHWA.Areas.Reports.Controllers
{
    ...
    public class rptFileSummaryByContactsController : Controller
    {
        ...
        public ActionResult PDF(int? employeeKey, string labelCriteria, string strWhere)
        {
            ...
            lr.ReportPath = "Areas/Reports/ReportDesign/rptFileSummaryByContacts.rdlc";
                ...
                string sql = @"WITH qData
                                ...
                                (
                                   ...
                                   FROM qrptFileSummary a
                                        inner join tblFileHeader fh on a.FileKey = fh.FileKey
                                   WHERE {1} 
                                )
                                SELECT a.*
                                    ,ISNULL(SQ.Total,0) as TotalSQ
                                    ,ROW_NUMBER() OVER (ORDER BY a.IsClosed DESC, a.CustName, a.ContactName, a.FileYear DESC, a.FileNum Desc) as Row
                                FROM qData a 
                                    OUTER APPLY (SELECT COUNT(*) as Total FROM dbo.qrptFileSummarySubQuotes b WHERE b.QHdrFileKey = a.FileKey) as SQ
                                ORDER BY Row;

                               DELETE FROM tblReportCriteria WHERE CriteriaRptName = 'rptFileSummaryByContacts' and CriteriaEmployeeKey = {2}";
