...
{
    [TokenValidation]
    public class qlstScheduleBImportsController : ApiController
    {
        ...
        public object GetAll()
        {
            ...
            int id = 0;
            int.TryParse(queryValues["id"], out id);
            ...
            try
            {
                if (id == 0)
                {
                    ...
                    IList<qlstScheduleBImport> lista = repository.GetListScheduleB(query, sort, filter, queryBy, page, start, limit, ref totalRecords);
                    json = new
                    {
                        total = totalRecords,
                        data = lista,
                ...
                {
                    qlstScheduleBImport item = repository.GetScheduleB(id);
                    var lista = new List<qlstScheduleBImport>
                    {
                       item
                    };
